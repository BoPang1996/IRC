...
def make_model(config):
    ...
    if config["model_points"] == "voxelnet":
        ...
        if config['momentum_k']:
            model_points.append(VoxelNet(4, config["model_n_out"], config))
    else:
        model_points = [MinkUNet(1, config["model_n_out"], config)]
        if config['momentum_k']:
            model_points.append(MinkUNet(1, config["model_n_out"], config))
    if len(model_points) > 1:
        for param_q, param_k in zip(model_points[0].parameters(), model_points[1].parameters()):
