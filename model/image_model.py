...
def adapt_weights(architecture):
    ...
    if not os.path.exists(f"weights/{architecture}.pt") and not os.path.exists(f"SLidR-main/weights/{architecture}.pt"):
        print("downloading weights: " + str(architecture))
        r = requests.get(weights_url[architecture], allow_redirects=True)
        os.makedirs("weights", exist_ok=True)
        with open(f"weights/{architecture}.pt", 'wb') as f:
            f.write(r.content)

    if os.path.exists(f"weights/{architecture}.pt"):
        weights = torch.load(f"weights/{architecture}.pt")
    else:
        weights = torch.load(f"SLidR-main/weights/{architecture}.pt")
...
class DilationFeatureExtractor(nn.Module):
    ...
    def __init__(self, config, preprocessing=None):
        ...
        self.decoder_cycle = nn.Sequential(
            nn.Conv2d(in1, config["model_n_out"], 1),
            nn.Upsample(scale_factor=4, mode="bilinear", align_corners=True),
        )
    ...
    def forward(self, x):
        ...
        out_cycle = self.decoder_cycle(feature)
        if self.normalize_feature:
            out = F.normalize(out, p=2, dim=1)
            out_cycle = F.normalize(out_cycle, p=2, dim=1)
        return out, out_cycle
