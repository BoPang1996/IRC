...
class Res16UNetBase(ResNetBase):
    ...
    def network_initialization(self, in_channels, out_channels, config, D):
        ...
        self.final = self._make_final(out_channels, D, bn_momentum, config)
        self.final_cycle = self._make_final(out_channels, D, bn_momentum, config)
        if config['triple_head_standalone']:
            self.final_triple = self._make_final(out_channels, D, bn_momentum, config)

        self.relu = MinkowskiReLU(inplace=True)

    def _make_final(self, out_channels, D, bn_momentum, config):
        if not config['mlp_head']:
            return conv(self.PLANES[7], out_channels, kernel_size=1, stride=1, bias=True, D=D)
        else:
            return torch.nn.Sequential(
                conv(self.PLANES[7], self.PLANES[7], kernel_size=1, stride=1, bias=False, D=D),
    ...
    def forward(self, x):
        ...
        out_feat = self.block8(out)

        out = self.final(out_feat)

        out_cycle = self.final_cycle(out_feat.detach() if self._config['detach_cycle_head'] else out_feat)
        if hasattr(self, 'final_triple'):
            out_triple = self.final_triple(out_feat)
        else:
            out_triple = self.final_cycle(out_feat)
        ...
        if self.normalize_feature:
            return SparseTensor(
                   ...
                   SparseTensor(
                       out_cycle.F / torch.norm(out_cycle.F, p=2, dim=1, keepdim=True),
                       coordinate_map_key=out_cycle.coordinate_map_key,
                       coordinate_manager=out_cycle.coordinate_manager,
                   ), \
                   SparseTensor(
                       out_triple.F / torch.norm(out_triple.F, p=2, dim=1, keepdim=True),
                       coordinate_map_key=out_triple.coordinate_map_key,
                       coordinate_manager=out_triple.coordinate_manager,
                   )
        else:
            return out, out_cycle, out_triple
